...
# Attribute : A descriptor or characterstic i.e. Data
...
class Shirt:
    # class definition always contains __init__() method for data inside the class.
    ...
    # all the self.attributes of a given object are accessible inside the method with the "self" keyword
...
# class Gaussian():
#     '''gaussain distribution class for visualizing a gaussian distribution'''
...
#     def claculate_stdev(self, sample=True):
#         '''Calculates std deviation of data set'''
#         if sample:
#             n = len(self.data)-1 # fo justification of taking n=n-1 for sample data see the other files folder
#         else:
#             n=len(self.data)
#         mean = self.mean
#         sigma=0
...
#     def read_data_file(self, file_name,smaple=True):
#         '''Method to read data from text file. The rext file should have one number(float) per line. The numbers are stored in data attribute. After file reading mean and std deviation are calculated'''
...
#         self.stdev=self.claculate_stdev(sample)
    
#     def plot_histogram(self):
#         '''Method to ouput histogram of instance variable data using matplolib pyplot library'''
#         plt.hist(self.data)
...
#     def plt_histogram_pdf(self,n_spaces=50):
...
class Pants(Clothing):
    ...
    def calculate_discount(self,discount):  # overrides Clothing.calculate_discount()
...
#   def calculate_mean(self):
    ...
    #     """function to calcualte p and n from the data set
    ...
    #         Args:
    #             other (Binomial)L Binomial instance
